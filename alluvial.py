...
class AlluvialTool:
    def __init__(
        ...
        # Describes pairs of labels from 2 categories (e.g. "country" - "spoken language")
        ...
        # Defines the horizontal distance between matplotlib patches (relative to x_range)
        self.h_gap_frac = h_gap_frac
        # Defines the vertical distance between matplotlib patches (relative to max y)
        self.v_gap_frac = v_gap_frac
        self.vein_blueprint = self.make_vein_blueprint_arrays()
        # TODO: Consider explicitly passing input_data
    ...
    def get_ordered_label_groups(self, src_label_override=None, dst_label_override=None, **kwargs):
        ...
        dst_ordered_labels = dst_label_override if dst_label_override else sorted(
            {dst_item for dst_item_counter in self.input_data_dict.values()
             for dst_item in dst_item_counter},
    ...
    def generate_alluvial_vein(self, src_item, dst_item):
        ...
        src_item_coord = self.item_coord_dict[src_item].read_state_and_advance_y(
            width)
        dst_item_coord = self.item_coord_dict[dst_item].read_state_and_advance_y(
            width)
        ...
        for src_item in self.src_group:
            ...
            for dst_item in self.dst_group:
                if dst_item in dst_items_for_current_src_item:
                    src_label_rect, dst_label_rect = self.get_label_rectangles_xy(
                        src_item, dst_item)
    ...
    def plot(self, figsize=(10, 15), alpha=0.5, **kwargs):
        ...
        for ind, vein_items in enumerate(self.alluvial_fan.values()):
            patches = [Polygon(item, facecolor=colors[ind], alpha=alpha)
    ...
    def get_vein_label_lengths(self):
        item_text_len = max([len(it)
                            for it in self.item_width_dict], default=0)
        width_text_len = max([len(str(w))
                             for w in self.item_width_dict.values()], default=0)
